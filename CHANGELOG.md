...
- **Dates and hours are UTC throughout.** The date-range bounds (`getRangeBounds`) were built from local calendar components while the underlying turns are bucketed by the UTC portion of their timestamp, so the client and the data sat on different calendars for the part of each day the two disagree: west of UTC, "Today" and "This Month" selected a day the stored data had not reached yet, and the Monthly Spend Limit panel totalled the wrong window. Every bound is now derived with `getUTC*` / `Date.UTC`, and the range selector carries a **UTC** marker. Usage after 20:00 US-Eastern still lands on the next day's bar — that's the UTC day boundary, and it's the one claude.ai bills on.
- The hourly distribution chart's **Local / UTC toggle is gone** and its hour axis is fixed to **Eastern**, labelled on the axis title, the day count, and the tooltip. This is the one deliberate exception to the UTC calendar: an hour-of-day chart answers when during the day you work, which is a question about a real timezone rather than a billing boundary. Hours resolve through `Intl.DateTimeFormat` per instant, so daylight saving and its transition days are handled rather than approximated with a fixed offset, and the zone is hardcoded rather than read from the viewer so the chart reads the same everywhere. Day buckets and every date range remain UTC.
- **Removed the peak-hour band** from the hourly chart. It shaded Mon–Fri 05:00–11:00 PT, marking the peak-hour limit reduction Anthropic applied to Claude Code — [which Anthropic removed for Pro and Max on 2026-05-06](https://www.anthropic.com/news/higher-limits-spacex), about two weeks after the band shipped upstream (#72). It has been highlighting a policy that no longer exists ever since, and it never described the viewer's own peak. The `PEAK_HOURS_UTC` constant was also pinned to PDT, so it shaded the wrong bars for roughly five months a year while the policy did exist.
- The claude.ai "as of" stamp and the CSV export filename are UTC, and the reconciliation caption renders the stamp the same way the sessionKey panel does.
...
- `today` and `week` now anchor on the **UTC** date (`utc_today()`) instead of the local one, and label it. West of UTC, `today` after 20:00 local queried a UTC day the stored turns hadn't reached and reported near-zero usage. `fetch-spend`'s default month window anchors on UTC for the same reason.
...
- Documented the UTC day convention as a fourth non-obvious invariant in AGENTS.md, and replaced the guess about the claude.ai API's day boundary with what the data shows: fitting local cost against `api_spend` under "local can never exceed billed" leaves UTC…UTC+2 feasible and rules out US zones.
- New [tests/test_timezone.py](tests/test_timezone.py) forces UTC-12 and UTC+14 so a local-date regression fails the suite. CI runs in UTC, where every one of these bugs is invisible.
- AGENTS.md gains a **Pulling in upstream changes** section covering the non-obvious parts of a fork merge: merge rather than rebase, the recurring `TBD` version-heading collision, upstream tags arriving in this clone while `origin` has none, upstream fixes that are wrong under this fork's conventions, `calcCost`'s extra 1h-cache argument that fails silently, and that GitHub Actions never run on this fork despite reporting as enabled.
- Merged upstream `v1.5.5`. The Est. Cost overlay it adds to the Daily Token Usage chart prices the 5m/1h cache-write split, matching the rest of this fork's cost math.
