...
- The hourly distribution chart's **Local / UTC toggle is gone**; hours are always UTC, labelled as such on the day count and in the tooltip. In local mode the chart shifted its hour buckets while still counting UTC days, so the per-hour averages were divided by a day count that didn't match the buckets.
