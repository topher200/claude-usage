...
- The hourly distribution chart's **Local / UTC toggle is gone** and its hour axis is fixed to **Eastern**, labelled on the axis title, the day count, and the tooltip. This is the one deliberate exception to the UTC calendar, and it exists to make the peak band correct: Anthropic's throttling window is defined as Mon–Fri 05:00–11:00 PT, which is a fixed **08:00–14:00 ET** all year because Pacific and Eastern shift together, while the equivalent UTC window moves between 12–17 and 13–18. `PEAK_HOURS_UTC = [12..17]` was hardcoded to PDT and so highlighted the wrong bars for roughly five months a year. Hours now resolve through `Intl.DateTimeFormat` per instant, so daylight saving — including the transition days — is handled rather than approximated. Day buckets and every date range remain UTC; only the hour axis is Eastern.
